document.addEventListener("DOMContentLoaded", async () => {
  try {
    ...
    if (hero) {
      document.getElementById("hero-bg")?.style.setProperty(
        "background-image",
      ...
      data.socials.forEach(social => {
        socialsContainer.insertAdjacentHTML("beforeend", `
          <a href="${social.link}" target="_blank" 
             class="flex flex-col items-center justify-center bg-gray-800 hover:bg-red-600 transition p-6 rounded-xl shadow-lg">
            <i class="${social.icon} text-3xl mb-3"></i>
            <span class="text-gray-200 font-semibold">${social.name}</span>
          </a>
        `);
      });
    }
